...
public sealed class Submission : AggregateRoot
{
    ...
    public Submission(AggregateId id, ConferenceId conferenceId, string title, string description, int level,
        string status, IEnumerable<string> tags, ICollection<Speaker> speakers, int version = 0) : this(id,
        conferenceId)
    {
        Title = title;
    ...
    {
        var speakersList = speakers.ToList();
        if (speakers is null || !speakersList.Any())
        {
            throw new MissingSubmissionSpeakersException(Id);
        }

        _speakers = speakersList.ToList();
    }
